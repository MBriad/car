...
* 【主控芯片】： STC89C52
* 【编译环境】： Keil μVisio4
* 【程序功能】： 超声波与红外避障，1602液晶显示距离
* 【使用说明】： 1、必须接上1602液晶显示器
				 2、请将接在主控板和小车底板上的P32-P35的4P杜邦线拔掉
				 3、再准备两条杜邦线，分别把小车底盘上P34接到主控板的P10上，P35接到主控板的P11上

				 液晶第一行显示测距方向，F前方
				 第二行是显示的距离，单位毫米
				 可以通过这个显示信息来调节智能小车避障逻辑，理论上不同实验环境
				 下都需要调试才能达到理想效果。
				 执行逻辑app.c文件夹内UltrasonicAvoid函数
* 【注意事项】： 云台不防撞
*/
...
void main()
{
	...
	while(1)
	{
		UltrasonicAvoid(300);//超声波避障，LCD1602显示距离,形参设置触发距离，单位：毫米
	}
} 