...
参数	：val设置避障触发距离
返回值	：无
描述	：智能小车黑线循迹加避障，当出现障碍物时，智能小车停车等待
障碍物移除后智能小车继续循迹
距离单位：毫米
====================================*/
void TrackingAvoid(uint val)
{
	uint Dis;//距离暂存变量
	...
	{	
		...
		BUZZER_off;//关闭蜂鸣器
		if((TrackSensorLeft == 0) && (TrackSensorRight == 0))
		{
			SmartCarBack(80, 80);//如果左右都出线了执行后退，回到黑线上
		}
	}
	else
	{
