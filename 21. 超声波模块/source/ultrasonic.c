...
void My_TrigUltrasonic()	//新超声的 触发
{
	My_TPin = 0; //超声波模块Trig	控制端
	_nop_();
	_nop_();
	_nop_();
	_nop_();
	_nop_();
	My_TPin = 1; //超声波模块Trig	控制端
	Delay15us();
	My_TPin = 0; //超声波模块Trig	控制端
}

unsigned int My_GetDistance()  //  新超声的距离获取
{
	unsigned int Distance = 0;	//超声波距离
	unsigned int  Time=0;		//用于存放定时器时间值
	My_TrigUltrasonic();	//超声波触发
	while(!My_EPin);  	//判断回响信号是否为低电平
	TR2 = 1;
	while(My_EPin);		//等待收到回响信号
	TR2 = 0;
	Time=TH2*256+TL2;	//读取时间
	TH2=0;
	TL2=0;				//清空定时器
    Distance = (float)(Time*1.085)*0.17;//算出来是MM
	return(Distance);//返回距离				
